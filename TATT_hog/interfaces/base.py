...
class HOGLayerC3(nn.Module):
    def __init__(self, nbins=4, pool=2, gaussian_window=0):
        super(HOGLayerC3, self).__init__()
        self.nbins = nbins
        self.pool = pool
        self.pi = math.pi
        weight_x = torch.FloatTensor([[1, 0, -1], [2, 0, -2], [1, 0, -1]])
...
class TextBase(object):
    def __init__(self, config, args, opt_TPG=None):
        ...
        self.hog = HOGLayerC().to(self.device)
        self.hog3 = HOGLayerC3().to(self.device)
        ...
        elif self.args.arch == 'tatt' or self.args.arch == 'tatt_mlp' or self.args.arch == 'tatt_att':
            self.align_collate = alignCollate_realWTLAMask
            self.load_dataset = lmdbDataset_real
        ...
        alpha_dict = {
            ...
            'chinese': open("/home/zwy/sr/TATT/al_chinese.txt", "r").readlines()[0].replace("\n", "")
        }
    ...
    def generator_init(self, iter=-1, resume_in=None):
        ...
        if self.args.arch == 'tsrn':
            ...
            model = tsrn.TSRN_TL_TRANS(scale_factor=self.scale_factor, width=cfg.width, height=cfg.height,
                                 STN=self.args.STN, mask=self.mask, srb_nums=self.args.srb,
                                 hidden_units=self.args.hd_u,pga=self.args.pga)
            image_crit = image_loss.ImageLoss(gradient=self.args.gradient, loss_weight=[1, 1e-4])
        elif self.args.arch in ['tatt_mlp']:
            model = tsrn.TSRN_TL_WAVE(scale_factor=self.scale_factor, width=cfg.width, height=cfg.height,
                                 STN=self.args.STN, mask=self.mask, srb_nums=self.args.srb,
                                 hidden_units=self.args.hd_u)
            image_crit = image_loss.ImageLoss(gradient=self.args.gradient, loss_weight=[1, 1e-4])
        elif self.args.arch in ['tatt_att']:
            model = tsrn.TSRN_TL_ATT(scale_factor=self.scale_factor, width=cfg.width, height=cfg.height,
                                 STN=self.args.STN, mask=self.mask, srb_nums=self.args.srb,
                                 hidden_units=30)
            image_crit = image_loss.ImageLoss(gradient=self.args.gradient, loss_weight=[1, 1e-4])
        elif self.args.arch == "tsrn_tl_cascade_sft":
