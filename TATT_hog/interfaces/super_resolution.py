...
ABLATION_SET = ["tsrn_tl_cascade_sft", "tsrn_tl_cascade", "srcnn_tl",
                ...
                "esrgan_tl", "scgan_tl", "tbsrn_tl", "tatt", "pcan_tl","tatt_mlp","tatt_att"]
                    ...
                    if self.args.arch == "tsrn":
                        ...
                        for i in range(self.args.stu_iter):
                            ...
                            if self.args.ssim_loss:
                                ...
                                loss_img += loss_ssim
                            if self.args.hog_loss:
                                # hog_sr = []
                                # hog_hr = []
                                # for z in  cascade_images.cpu().detach().numpy():
                                #     _, hog_srimage = hog(z,
                                #                             orientations=9,
                                #                             pixels_per_cell=(4, 4),
                                #                             cells_per_block=(2, 2),
                                ...
                                #     hog_sr.append(hog_srimage)
                                # for l in  images_hr.cpu().detach().numpy():
                                #     _, hog_hrimage = hog(l,
                                #                             orientations=9,
                                #                             pixels_per_cell=(4, 4),
                                #                             cells_per_block=(2, 2),
                                #                             block_norm='L2-Hys',
                                #                             visualize=True, multichannel=True)
                                #     hog_hr.append(hog_hrimage)

                                # hog_loss = (np.array(hog_sr)-np.array(hog_hr))**2
                                # hog_loss = hog_loss.mean()*100
                                # # print('hog_loss0:',hog_loss)
                                # # print('loss_img1:',loss_img)
                                # loss_img += hog_loss
                                # # print('loss_img2:',loss_img)

                                hog_sr = self.hog(cascade_images)
                                hog_hr = self.hog(images_hr)
                                # hog_sr = self.hog3(cascade_images[:, :3, :, :])
                                # hog_hr = self.hog3(images_hr[:, :3, :, :])
                                
                                # hog_loss = (hog_sr-hog_hr)**2
                                # hog_loss = hog_loss.mean()*300

                                hog_loss = self.l1(hog_sr,hog_hr)*500
                                # print('hog_loss0:',hog_loss)
                                # print('loss_img1:',loss_img)
                                loss_img += hog_loss
                            ...
                            if iters % 5 == 0 and i == self.args.stu_iter - 1:
                                ...
                                #     self.results_recorder.add_scalar('loss/hog_loss', float(hog_loss) * 100,
                                #                                     global_step=iters)
